...
def get_decay_from_ewc_mode(spec):
    """
    Only return decay factor if usig Online EWC.
    """
...
config_cl ={'Replay':{'mem_size':tune.choice([4,16,32])},
            ...
            'EWC':{'ewc_lambda':tune.choice([1e-3,1e-2,1e-1,1e0,1e1,1e2]), 'mode':tune.choice(['separate','online']), 'decay_factor':tune.sample_from(get_decay_from_ewc_mode)},
